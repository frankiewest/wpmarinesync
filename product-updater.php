...
function update_woocommerce_products_from_xml() {
	...
	if (!$xml) {
		error_log("Rightboat XML Import: Failed to parse XML from $xml_feed_url");
		return;
	}
	...
	foreach ($adverts as $advert) {
		...
		if (!empty($status)) {
			if ($status === "Sold") {
				...
				if ($sold_status) {
					wp_remove_object_terms($boat, $sold_status->term_id, 'boat-status');
					error_log("Sold status removed from the boat.");
		...
		$media_importer = new class($boat_images, $boat_id, 'marinesync-media-importer') {
			...
			public function processImages() {
				if (empty($this->images) || !is_array($this->images)) {
					error_log("MediaImporter::processImages - No images to process for post {$this->boat_id}");
					return;
				}
				...
				if ($first_image) {
					...
					if ($thumbnail_id) {
						...
						// Include featured image in gallery
				...
				$field_key = 'field_674eea588a88f'; // Replace with actual field key for 'boat_media'
				if (!empty($gallery_attachment_ids)) {
					// Merge with existing gallery images
					$existing_gallery = get_field('boat_media', $post_id, false) ?: [];
					if (!is_array($existing_gallery)) {
						$existing_gallery = [];
					}
					$updated_gallery = array_unique(array_merge($existing_gallery, $gallery_attachment_ids));

					$result = update_field($field_key, $updated_gallery, $post_id);
					if ($result) {
						error_log("MediaImporter::processImages - Updated ACF gallery field '$field_key' with " . count($updated_gallery) . " images for post $post_id");
					} else {
						error_log("MediaImporter::processImages - Failed to update ACF gallery field '$field_key' for post $post_id");
					}
				} else {
					error_log("MediaImporter::processImages - No attachment IDs to update ACF gallery field for post $post_id");
				}
			}
		};
