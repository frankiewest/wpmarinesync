...
function marinesync_custom_search_join($join) {
	...
	// Avoid duplicates
	if (strpos($join, "{$wpdb->postmeta} AS mt1") === false) {
		$join .= " LEFT JOIN {$wpdb->postmeta} AS mt1 ON ({$wpdb->posts}.ID = mt1.post_id)";
	}
	if (strpos($join, "{$wpdb->postmeta} AS mt2") === false) {
		$join .= " LEFT JOIN {$wpdb->postmeta} AS mt2 ON ({$wpdb->posts}.ID = mt2.post_id)";
	}
	if (strpos($join, "{$wpdb->postmeta} AS mt3") === false) {
		$join .= " LEFT JOIN {$wpdb->postmeta} AS mt3 ON ({$wpdb->posts}.ID = mt3.post_id)";
	}
	return $join;
}

// Custom WHERE to match post_title and ACF fields
function marinesync_custom_search_where($where) {
	global $wpdb;
...
// Group results by post ID to prevent duplicates
function marinesync_custom_search_groupby($groupby) {
	global $wpdb;
	if (!$groupby) {
